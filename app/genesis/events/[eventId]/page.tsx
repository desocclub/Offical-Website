...
const EVENTS: Record<string, EventDetailConfig> = {
  hackathon: {
    name: 'Hackathon',
...
export async function generateMetadata({ params }: PageProps) {
  ...
  return {
    title: event ? `${event.name} — Genesis | DESOC` : 'Event Not Found — Genesis | DESOC',
    description: event?.description || 'Genesis event details',
  };
}

export default async function GenesisEventPage({ params }: PageProps) {
  const { eventId } = await params;
  const event = EVENTS[eventId];
      ...
      <div className="relative">
        ...
        <main className="relative px-4 sm:px-6 lg:px-8 pt-24 pb-16 max-w-4xl mx-auto">
          ...
          <header className="text-center mb-10">
            ...
            <h1 className="heading-title-6 text-primary font-medium-510 text-center leading-tight mb-2">
              {event.name}
            </h1>
          ...
          <div className="space-y-5">
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-2">
              <Link
                href={`/genesis/register?event=${encodeURIComponent(event.registrationKey)}`}
